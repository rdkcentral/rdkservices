...
//=========================================== HdmiCecSource =========================================
	...
	void HdmiCecSource::sendOTPMsg()
	{
		...
		LOGINFO("Command: sending ImageViewOn TV \r\n");
                _instance->smConnection->sendTo(LogicalAddress::TV, MessageEncoder().encode(ImageViewOn()));
                usleep(10000);
                LOGINFO("Command: sending ActiveSource  physical_addr :%s \r\n",physical_addr.toString().c_str());
                _instance->smConnection->sendTo(LogicalAddress::BROADCAST, MessageEncoder().encode(ActiveSource(physical_addr)));
                usleep(10000);
                isDeviceActiveSource = true;
                LOGINFO("Command: sending GiveDevicePowerStatus \r\n");
                _instance->smConnection->sendTo(LogicalAddress::TV, MessageEncoder().encode(GiveDevicePowerStatus()));
	}
