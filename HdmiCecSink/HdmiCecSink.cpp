...
#define HDMICECSINK_METHOD_GET_CEC_VERSION       "getCecVersion"
#define HDMICECSINK_METHOD_SET_CEC_VERSION       "setCecVersion"
...
//=========================================== HdmiCecSink =========================================
       ...
       : PluginHost::JSONRPC()
       {
		  ...
		  Register(HDMICECSINK_METHOD_REQUEST_ACTIVE_SOURCE, &HdmiCecSink::requestActiveSourceWrapper, this);
		   ...
		   Register(HDMICECSINK_METHOD_GET_CEC_VERSION,&HdmiCecSink::getCECVersionWrapper,this);
                   Register(HDMICECSINK_METHOD_SET_CEC_VERSION,&HdmiCecSink::setCECVersionWrapper,this);
        ...
        uint32_t HdmiCecSink::requestShortAudioDescriptorWrapper(const JsonObject& parameters, JsonObject& response)
	{
	  ...
	  returnResponse(true);
	   ...
	   uint32_t HdmiCecSink::getCECVersionWrapper(const JsonObject& parameters, JsonObject& response)
	   {
              bool resp = true;
              response["cecVersion"] = cecVersion;
              returnResponse(resp);
	    }
	    uint32_t HdmiCecSink::setCECVersionWrapper(const JsonObject& parameters, JsonObject& response)
	    {
               LOGINFOMETHOD();
               returnIfParamNotFound(parameters, "cecVersion");
               string cec_version = parameters["cecVersion"].String();
               bool success = true;
               LOGINFO("HdmiCecSink::cecVersion  :%s \n", cec_version.c_str());

               if(cecVersion != stof(cec_version)) {
                  cecVersion = stof(cec_version);
                  JsonObject params;
                  params["cecVersion"] = cec_version;
                  //write to persistence storage
                  WDMP_STATUS status = setRFCParameter((char*)"thunderapi",
                       TR181_HDMICECSINK_CEC_VERSION,cec_version.c_str(),WDMP_STRING);
                  if ( WDMP_SUCCESS == status ) {
                     LOGINFO("Set CEC Version Success\n");
                  }
                  else {
                     LOGINFO("Set CEC Version Failed %s\n",getRFCErrorString(status));
                  }
              }
