	...
	void HdmiCecSink::updateActiveSource(const int logical_address, const ActiveSource &source )
       	{
			...
			if ( _instance->m_logicalAddressAllocated == LogicalAddress::UNREGISTERED ){
				...
				return;
                    ...
                    _instance->smConnection->sendTo(LogicalAddress::AUDIO_SYSTEM,MessageEncoder().encode(RequestShortAudioDescriptor(formatid,audioFormatCode,numberofdescriptor)), 500);
        ...
        void HdmiCecSink::Send_Request_Arc_Initiation_Message()
	{
	     ...
	     return;
          ...
          LOGINFO(" Send_Request_Arc_Initiation_Message ");
           _instance->smConnection->sendTo(LogicalAddress::AUDIO_SYSTEM,MessageEncoder().encode(RequestArcInitiation()), 500);
	    ...
	    return;
            if(!(_instance->smConnection))
               return;
            _instance->smConnection->sendTo(LogicalAddress::AUDIO_SYSTEM,MessageEncoder().encode(ReportArcInitiation()), 500);
        ...
        void HdmiCecSink::Send_Request_Arc_Termination_Message()
        {
	     ...
	     return;
            if(!(_instance->smConnection))
               return;
            _instance->smConnection->sendTo(LogicalAddress::AUDIO_SYSTEM,MessageEncoder().encode(RequestArcTermination()), 500);
        }
       ...
       void HdmiCecSink::Send_Report_Arc_Terminated_Message()
       {
		...
		return;
            if(!(_instance->smConnection))
               return;
           _instance->smConnection->sendTo(LogicalAddress::AUDIO_SYSTEM,MessageEncoder().encode(ReportArcTermination()), 500);
