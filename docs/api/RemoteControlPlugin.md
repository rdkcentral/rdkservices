...
| [startFirmwareUpdate](#startFirmwareUpdate) | Starts a firmware image update session for the specified remote |
...
Starts a firmware image update session for the specified remote.
...
| result | object |  |
| result.status | object |  |
| result.status.sessionId | string | The unique identifier for the firmware update session, generated by the underlying RDK stack |
| result.success | boolean | Whether the request succeeded |
...
| result.status.result.result | string | The result of the firmware update session (`pending`, `success`, `error`, `abort`). This also determines which object will be in the parameters (`error`, `abort`) |
| result.status.result?.error | object | <sup>*(optional)*</sup> This optional object is included only if the `result` value is `error` |
| result.status.result?.error.errorCode | integer | The return code from the underlying RDK stack |
| result.status.result?.abort | object | <sup>*(optional)*</sup> This optional object is included only if the `result` value is `abort` |
| result.status.result?.abort.reason | integer | The reason code that identifies why the session was aborted: `0` - User (must be one of the following: *0*) |
| result.success | boolean | Whether the request succeeded |
...
| params.result.result | string | The result of the firmware update session (`pending`, `success`, `error`, `abort`). This also determines which object will be in the parameters (`error`, `abort`) |
| params.result?.error | object | <sup>*(optional)*</sup> This optional object is included only if the `result` value is `error` |
| params.result?.error.errorCode | integer | The return code from the underlying RDK stack |
| params.result?.abort | object | <sup>*(optional)*</sup> This optional object is included only if the `result` value is `abort` |
| params.result?.abort.reason | integer | The reason code that identifies why the session was aborted: `0` - User (must be one of the following: *0*) |
