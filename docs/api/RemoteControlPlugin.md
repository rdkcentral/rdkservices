...
| [unpair](#unpair) | Unpairs a remote given a mac address
...
| result.status.remoteData[#]?.upgradeSessionId | string | <sup>*(optional)*</sup> The unique identifier for the firmware update session, generated by the underlying RDK stack |
| result.success | boolean | Whether the request succeeded |
...
| result.status | object |  |
| result.status.| object |  |
| result.status.macAddress | string | The MAC address of the remote in hex-colon format |
| result.status.percentComplete | integer | The estimated percentage of the firmware update that has completed in percent (0 - 100) |
| result.status.upgradeState | string | The state of the firmware update session (`idle`, `pending`, `success`, `error`, `canceled`, `invalid`)
| result.status.errorString? | string | <sup>*(optional)*</sup> The return error string from the underlying RDK stack |
| result.success | boolean | Whether the request succeeded |
...
| params.status.remoteData[#]?.upgradeSessionId | string | <sup>*(optional)*</sup> The unique identifier for the firmware update session, generated by the underlying RDK stack |
...
| result.status.upgradeState | string | The state of the firmware update session (`idle`, `pending`, `success`, `error`, `canceled`, `invalid`)
| params.errorString? | string | <sup>*(optional)*</sup> The return error string from the underlying RDK stack |
