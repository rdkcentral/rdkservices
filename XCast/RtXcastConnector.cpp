...
#define NETFLIX_DEF_APP_REQ ("{\"names\":[\"Netflix\"],\"cors\":[\".netflix.com\"],\"properties\":{\"allowStop\" :true}}")
#define YOUTUBE_DEF_APP_REQ ("{\"names\":[\"YouTube\"],\"cors\":[\".youtube.com\"],\"properties\":{\"allowStop\" :true}}")
#define SPOTIFY_DEF_APP_REQ ("{\"names\":[\"com.spotify.Spotify.TV\"],\"prefixes\" : [\"com.spotify\"],\"cors\":[\".spotify.com\"],\"properties\":{\"allowStop\" :true}}")
#define PARING_DEF_APP_REQ  ("{\"names\":[\"Pairing\"],\"cors\":[\".comcast.com\"],\"properties\":{\"allowStop\" :true}}")
...
int RtXcastConnector::connectToRemoteService()
{
    ...
    if(err == RT_OK && xdialCastObj != NULL)
    {
       ...
       /*Static AppList init*/
       if(!IsDynamicAppListEnabled()) {
           char strXCastDefaultAppReq [1024];
           bool IsCommaReq = false;

           memset (strXCastDefaultAppReq, '\0', sizeof (strXCastDefaultAppReq));
           strcpy (strXCastDefaultAppReq, "[");
           if(IsAppEnabled("youtube")) {
               strcat (strXCastDefaultAppReq, YOUTUBE_DEF_APP_REQ);
               IsCommaReq = true;
           }
           if(IsAppEnabled("spotify")) {
               if (IsCommaReq) {
                   strcat (strXCastDefaultAppReq, ", ");
               }
               strcat (strXCastDefaultAppReq, SPOTIFY_DEF_APP_REQ);
               IsCommaReq = true;
           }
           if(IsAppEnabled("netflix")) {
               if (IsCommaReq) {
                   strcat (strXCastDefaultAppReq, ", ");
               }
               strcat (strXCastDefaultAppReq, NETFLIX_DEF_APP_REQ);
               IsCommaReq = true;
           }
           if(IsAppEnabled("pairing")) {
               if (IsCommaReq) {
                   strcat (strXCastDefaultAppReq, ", ");
               }
               strcat (strXCastDefaultAppReq, PARING_DEF_APP_REQ);
               IsCommaReq = true;
           }
           strcat (strXCastDefaultAppReq, "]");
           std::string strAppList = std::string (strXCastDefaultAppReq);
           registerApplications (strAppList);
       }
...
bool RtXcastConnector::getEntryFrmAppLaunchParamList (const char* appName, RegAppLaunchParams* reqParam){
