...
, m_apiVersionNumber(1), m_isDynamicRegistrationsToDo(false)
{
    InitializeIARM();
...
bool XCast::getEntryFromAppLaunchParamList (const char* appName, DynamicAppConfig* reqParam){
...
void XCast::updateDynamicAppCache(string strApps)
{
    ...
    if (!strApps.empty()) {
        ...
        {lock_guard<mutex> lck(m_appConfigMutex);
            ...
            //If empty list is passed cache need to be cleared
            if(!appConfigList.size()){
    ...
    if (hasAppReq) {
       ...
       if(_rtConnector)
       {
           ...
           if(_rtConnector->IsDynamicAppListEnabled()) {
               ...
               m_isDynamicRegistrationsToDo = true;
...
void XCast::onLocateCastTimer()
{
    ...
    if (NULL != _rtConnector) {
        if (_rtConnector->IsDynamicAppListEnabled() && m_isDynamicRegistrationsToDo) {
        ...
        string strPayLoad, string strQuery, string strAddDataUrl)
{
    ...
    if(_rtConnector) {
        ...
        getEntryFromAppLaunchParamList (appName.c_str(), &reqParam);
