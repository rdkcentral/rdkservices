...
#define CAT_DWNLDPROGRESSFILE_AND_GET_INFO "awk '/DOWN:/ { for (i=1; i<=NF; i++) if ($i==\"DOWN:\" && $(i+1) ~ /^[0-9]+$/ && $(i+3) ~ /^[0-9]+$/) { if ($(i+3) > 0) print int(($(i+1) * 100) / $(i+3)); else print -1; } }' /opt/curl_progress | tail -n 1"
