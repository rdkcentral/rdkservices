...
#define GET_STR(map, key) ((map.HasLabel(key) && map[key].String() != "null") ? map[key].String() : string())
...
namespace Plugin {
    ...
    uint32_t TextToSpeechImplementation::Configure(PluginHost::IShell* service)
    {
        ...
        ttsConfig->setVolume(std::stod(GET_STR(config, "volume")));
        ttsConfig->setRate(std::stoi(GET_STR(config, "rate")));
    ...
    uint32_t TextToSpeechImplementation::SetConfiguration(const string &input, string &output)
    {
        ...
        config.volume = std::stod(GET_STR(parameters, "volume"));
