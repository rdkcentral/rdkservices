...
namespace Plugin {
    ...
    uint32_t TextToSpeechImplementation::SetACL(const string method,const string apps)
    {
        ...
        _adminLock.Unlock();
        TTSLOG_INFO("setACL invoked with method %s",method);
        logResponse(status);
        return (status == TTS::TTS_OK) ? (Core::ERROR_NONE) : (Core::ERROR_GENERAL);
    }
    ...
    uint32_t TextToSpeechImplementation::Speak(const string text,const string callsign,uint32_t &speechid,Exchange::ITextToSpeech::TTSErrorDetail &ttsStatus)
    {
        CHECK_TTS_MANAGER_RETURN_ON_FAIL();
        _adminLock.Lock();
        speechid = nextSpeechId();
        auto status = _ttsManager->speak(speechid,text,callsign);
        ttsStatus = (Exchange::ITextToSpeech::TTSErrorDetail) status;
        _adminLock.Unlock();
