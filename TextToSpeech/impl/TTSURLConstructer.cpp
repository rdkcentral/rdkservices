...
std::string TTSURLConstructer::constructURL(TTSConfiguration &config,std::string text,bool isfallback)
{
    ...
    {
       if(config.isSwitchRequired())
       {
          if(WPEFramework::Plugin::NetworkStatusObserver::getNetworkStatusObserver()->isConnected())
          {
             TTSLOG_INFO("Device online using X-api based remote endpoint\n");
       ...
       {
           TTSLOG_INFO("Legacy device using endpoint configured based on x-api \n");
           return httppostURL(config,text,isfallback);
       }
    }
    else
    {
       ...
       {
            TTSLOG_INFO("Legacy device using endpoint configured\n");
           return httpgetURL(true,config,text,isfallback);
       }
