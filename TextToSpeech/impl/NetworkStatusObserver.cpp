...
NetworkStatusObserver* NetworkStatusObserver::getNetworkStatusObserver()
{
...
bool NetworkStatusObserver::isConnected()
{
    ...
    if (!m_eventRegistered) {
        if (m_networkService->Subscribe<JsonObject>(1000, "onConnectionStatusChanged",
                    &NetworkStatusObserver::onConnectionStatusChangedEventHandler, this) == Core::ERROR_NONE) {
            m_eventRegistered = true;
            TTSLOG_INFO("Subscribed to notification handler : onConnectionStatusChanged");
        } else {
            TTSLOG_ERROR("Failed to Subscribe notification handler : onConnectionStatusChanged");
        }
    }
...
{
    m_isConnected = (parameters.HasLabel("status") && parameters["status"].String() == "CONNECTED");
    TTSLOG_INFO("Network Interface State changed to %s",parameters["status"].String().c_str());
}
