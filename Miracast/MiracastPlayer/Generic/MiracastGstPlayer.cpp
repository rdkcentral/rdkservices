...
#if 1
static GstElement* m_soc_audio_sink = nullptr;
...
static GstElement* SoC_GetAudioSinkProperty(void)
{
    ...
    if ( nullptr == m_soc_audio_sink )
    {
        m_soc_audio_sink = gst_element_factory_make("amlhalasink", "miracast_amlhalasink");
        ...
        else
        {
            ...
            g_object_set(G_OBJECT(m_soc_audio_sink), "avsync-mode" , 2 , nullptr );
            ...
            g_object_set(G_OBJECT(m_soc_audio_sink), "disable-xrun" , true , nullptr );

            MIRACASTLOG_TRACE("amlhalasink configuration end <<<<<<<<");
        }
    }
    MIRACASTLOG_TRACE("Exiting ...");
    return m_soc_audio_sink;
}
...
static gboolean SoC_ReleaseAudioSinkProperty(GstElement* audioSink)
{
    ...
    MIRACASTLOG_TRACE("Entering ...");
    if (( nullptr == m_soc_audio_sink ) || ( m_soc_audio_sink != audioSink ))
    {
        MIRACASTLOG_WARNING("!!! Invalid Handle[%x] Current[%x] !!!",audioSink,m_soc_audio_sink);
    }
    else
    {
        gst_object_unref(m_soc_audio_sink);
        m_soc_audio_sink = nullptr;
        returnValue = true;
    }
...
void* MiracastGstPlayer::pushbuffer_handler_thread(void *ctx)
{
    MiracastGstPlayer *self = (MiracastGstPlayer *)ctx;
    MIRACASTLOG_TRACE("Entering..!!!");
    self->m_pushbuffer_handler_thread_loop = true;

    while (true == self->m_pushbuffer_handler_thread_loop)
    {
        usleep(100000);
    }
    self->m_pushbuffer_handler_tid = 0;
    MIRACASTLOG_TRACE("Exiting..!!!");
    pthread_exit(nullptr);
}
...
GstFlowReturn MiracastGstPlayer::appendPipelineNewSampleHandler(GstElement *elt, gpointer userdata)
{
    ...
    if ((nullptr == self) || (nullptr == self->m_appsrc))
    {
    ...
    lock_guard<mutex> lck(self->m_QueueStateMutex);
...
gboolean MiracastGstPlayer::pushBufferToAppsrc(gpointer userdata)
{
    ...
    if (nullptr == self)
    {
        MIRACASTLOG_ERROR("NULL Instance");
        return GST_FLOW_OK;
    }
...
bool MiracastGstPlayer::stop()
{
    ...
    {
        MIRACASTLOG_INFO("Stop PushBuffer Handler Thread");
        m_pushbuffer_handler_tid = false;
        lock_guard<mutex> lck(m_QueueStateMutex);
        if (m_customQueueHandle)
        {
            MIRACASTLOG_INFO("Cleanup msqueue");
