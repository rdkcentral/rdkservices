...
const string WPEFramework::Plugin::RDKShell::RDKSHELL_METHOD_CHECK_FOR_FTA_LAUNCH = "willLaunchFactoryAppAtStartup";
...
namespace WPEFramework {
    namespace Plugin {
                ...
                : AbstractPlugin(API_VERSION_NUMBER_MAJOR), mClientsMonitor(Core::Service<MonitorClients>::Create<MonitorClients>(this)), mEnableUserInactivityNotification(true), mCurrentService(nullptr), mLastWakeupKeyCode(0), mLastWakeupKeyModifiers(0), mLastWakeupKeyTimestamp(0)
        {
            ...
            registerMethod(RDKSHELL_METHOD_CHECK_FOR_FTA_LAUNCH, &RDKShell::willLaunchFactoryAppAtStartupWrapper, this);
      ...
      uint32_t RDKShell::showWatermarkWrapper(const JsonObject& parameters, JsonObject& response)
        ...
        uint32_t RDKShell::willLaunchFactoryAppAtStartupWrapper(const JsonObject& parameters, JsonObject& response)
        {
            LOGINFOMETHOD();
            response["enabled"] = checkForBootupFactoryAppLaunch();
            returnResponse(true);
        }
