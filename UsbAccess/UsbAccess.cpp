...
const string WPEFramework::Plugin::UsbAccess::REGEX_PATH = "^\\/([^~`.,!@#$%^&*:;()+={}<>\\[\\]\\s]+\\/)+$"; // here we are restriced Special Characters like (~`.,!@#$%^&*()+={}<>[]\\s(space))
...
namespace Plugin {
    ...
    namespace {
        ...
        int runScript(const string path, const string name) {
            int result = v_secure_system("/lib/rdk/userInitiatedFWDnld.sh %s '%s' '%s' %d >> /opt/logs/swupdate.log &","usb",path.c_str(),name.c_str(),0);
