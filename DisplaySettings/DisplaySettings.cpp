...
namespace WPEFramework {
	...
	uint32_t DisplaySettings::setForceHDRMode (const JsonObject& parameters, JsonObject& response)
        ...
        {   //sample servicemanager response: {"success":true}
            ...
            {
                device::VideoOutputPort &vPort = device::Host::getInstance().getVideoOutputPort(videoDisplay);
		if (0==strcmp(strColorDepth.c_str(), "8 Bit")){
			vPort.setPreferredColorDepth(dsDISPLAY_COLORDEPTH_8BIT, persist);
		} else if (0==strcmp(strColorDepth.c_str(), "10 Bit")){
			vPort.setPreferredColorDepth(dsDISPLAY_COLORDEPTH_10BIT, persist);
		} else if (0==strcmp(strColorDepth.c_str(), "12 Bit")){
			vPort.setPreferredColorDepth(dsDISPLAY_COLORDEPTH_12BIT, persist);
		} else if (0==strcmp(strColorDepth.c_str(), "Auto")){
			vPort.setPreferredColorDepth(dsDISPLAY_COLORDEPTH_AUTO, persist);
		} else {
			//UNKNOWN color depth
