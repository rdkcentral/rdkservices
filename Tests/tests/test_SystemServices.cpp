...
TEST_F(SystemServicesTest, getPlatformConfigurationSuccess_withEmptyQuery)
{
    ...
    const string SWUpdateConf = _T("/opt/swupdate.conf");
    Core::File file(SWUpdateConf);
    file.Create();

    EXPECT_TRUE(Core::File(string(_T("/opt/swupdate.conf"))).Exists());
