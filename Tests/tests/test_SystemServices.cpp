 ...
 *        then setPowerState will sets with provided state and setPowerState shall be succeed.
 *
 ...
 *        then it calls setPowerState with state as the parameter and setPowerState shall be succeed.
 *
 ...
 *         setPowerState shall be succeed.
 *
 * @param[in]   :  "params": {"powerState": "STANDBY"}
 * @return      :  {"success":true}
 */
 ...
 *          then it set the powerstate with prefered standbymode and setPowerState shall be succeed.
 *
 * @param[in]   :  "params": {"powerState": "STANDBY"}
 * @return      :  {"success":true}
 */
...
TEST_F(SystemServicesEventIarmTest, onSystemPowerStateChanged_PowerState_ON_To_LIGHTSLEEP)
{
    ...
    ON_CALL(rfcApiImplMock, getRFCParameter(::testing::_, ::testing::_, ::testing::_))
        .WillByDefault(::testing::Invoke(
            [](char* pcCallerID, const char* pcParameterName, RFC_ParamData_t* pstParamData) {
                EXPECT_EQ(string(pcParameterName), string("Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.LogUploadBeforeDeepSleep.Enable"));
...
TEST_F(SystemServicesTest, uploadLogsAsyncSuccess_WhenLogFileNotExist)
{
    EXPECT_FALSE(Core::File(string(_T("/lib/rdk/uploadSTBLogs.sh"))).Exists());
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("uploadLogsAsync"), _T("{}"), response));
}

TEST_F(SystemServicesTest, uploadLogsAsyncSuccess_WhenOptDcmFileNotExist)
{
    const string uploadStbLogFile = _T("/lib/rdk/uploadSTBLogs.sh");
    Core::File file(uploadStbLogFile);
    file.Create();
    ...
    EXPECT_FALSE(Core::File(string(_T("/opt/dcm.properties"))).Exists());
    ofstream dcmPropertiesFile("/etc/dcm.properties");
    dcmPropertiesFile << "LOG_SERVER=logs.xcal.tv\n";
