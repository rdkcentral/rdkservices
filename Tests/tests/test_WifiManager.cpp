...
{
    /*wpa_cli command output as file*/
    std::ofstream file("/tmp/wpa_cli.out");
    file << "bssid=ff:ff:ff:ff:ff:ff\nssid=rdk.test.wifi.network\nkey_mgmt=WPA2-PSK\nwpa_state=COMPLETED\nRSSI=-33\nLINKSPEED=130\nNOISE=-88\nFREQUENCY=2437\n";
    file.close();
    ...
    EXPECT_CALL(wrapsImplMock, popen(::testing::_, ::testing::_))
        ...
        .WillRepeatedly(::testing::Invoke(
            [&](const char* command, const char* type) {
                EXPECT_TRUE(string(command) == string(_T("wpa_cli status")) || string(command) == string(_T("wpa_cli signal_poll")));
                FILE *fptr = fopen("/tmp/wpa_cli.out","r");
                return fptr;
            }));
