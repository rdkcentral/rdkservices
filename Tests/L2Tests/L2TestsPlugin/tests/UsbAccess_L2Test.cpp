...
/********************************************************
************Test case Details **************************
** 1. Triggered getFileList Methods
** 2. Verify getFileList method getting triggered  w.r.t REGEX_FILE and verify the response accordingly
*******************************************************/
...
{  
    ...
    EXPECT_CALL(*p_wrapsImplMock, getmntent(testing::_))
        ...
        entry2.mnt_fsname = const_cast<char*>("/dev/sda2"); // Set the value for mnt_fsname
        entry2.mnt_dir = const_cast<char*>("/run/media/sda2/logs/");    // Set the value for mnt_dir
    ...
    std::vector<std::string> fileNames1 = {"file1.txt", "file2.txt", "FILE3.txt", "FILE4.png"};
...
** 2. Verify getAvailableFirmwareFiles & updateFirmware method getting triggered and verify the response
*******************************************************/
...
** 2. Verify getLinks, clearLink & createLink method getting triggered and verify the response
*******************************************************/
...
{  
    ...
    std::string directory = "/run/media/sda1/logs/";
    std::vector<std::string> fileNames = {"file1.txt"};
    for (const auto& fileName : fileNames){
    std::string filePath = directory + fileName;
    std::ofstream fileStream(filePath);
    }
    std::ofstream file("/run/media/sda1/logs/file1.txt");
    file << "MODEL_NUM=HSTP11MWR";
    file.close();

   
    status = InvokeServiceMethod("org.rdk.UsbAccess.1", "clearLink", params , res);
    EXPECT_EQ(Core::ERROR_GENERAL, status);

    status = InvokeServiceMethod("org.rdk.UsbAccess.1", "createLink", params , res);
    EXPECT_EQ(Core::ERROR_GENERAL, status);    
    ...
    status = InvokeServiceMethod("org.rdk.UsbAccess.1", "getLinks", params , res);    
    EXPECT_EQ(Core::ERROR_NONE, status);  
