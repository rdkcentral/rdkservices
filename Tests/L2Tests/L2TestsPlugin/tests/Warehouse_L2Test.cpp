...
** 2. To enable RFC_PWRMGR2 -- "Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.Power.PwrMgr2.Enable"
** 2. Triggered resetDevice Method 
** 3. Verify the response of resetDevice Method for different "resetType"
*******************************************************/
TEST_F(Warehouse_L2Test,Warehouse_Lcov_ResetDevice)
{
    JSONRPC::LinkType<Core::JSON::IElement> jsonrpc(WAREHOUSE_CALLSIGN,WAREHOUSEL2TEST_CALLSIGN );
    StrictMock<AsyncHandlerMock_Warehouse > async_handler;
    uint32_t status = Core::ERROR_GENERAL;
