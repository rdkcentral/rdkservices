...
TEST_F(SystemServicesTest, Mode)
{
    ...
    EXPECT_EQ(Core::ERROR_NONE, handler.Invoke(connection, _T("setMode"), _T("{\"modeInfo\":{\"mode\":\"EAS\",\"duration\":20}}"), response));
    EXPECT_EQ(response, string("{\"success\":true}"));
...
TEST_F(SystemServicesTest, requestSystemRebootSuccess_NrdPluginAppShutdownFailed)
{
     ...
     ON_CALL(*p_wrapsImplMock, system(::testing::StrEq("pkill nrdPluginApp")))
      .WillByDefault(::testing::Return(-1));
     EXPECT_CALL(*p_iarmBusImplMock, IARM_Bus_Call)
        .Times(1)
        .WillRepeatedly(
...
TEST_F(SystemServicesTest, clearLastDeepSleepReasonFailed_WhenPcloseFailed)
{
    ...
    EXPECT_CALL(*p_wrapsImplMock, pclose(fakePipe))
