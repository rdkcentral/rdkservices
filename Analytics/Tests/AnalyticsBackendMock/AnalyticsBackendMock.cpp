    ...
    public:
        ...
        uint32_t SendEvent(const Event& event) override {
            ...
            if (mStore) {
                ...
                if (mStore->RemoveEntries(TABLE_NAME,  startIndex, eventCount)) {
                    ...
                    while (eventCount > 0) {
                        ...
                        if (eventCount != 0)
                        {
                            LOGERR("There are still %d events left in local store after removal", eventCount);
                            mStore->RemoveEntries(TABLE_NAME,  startIndex, eventCount);
                        }
                    }
                    LOGINFO("All events removed from local store");
                } else {
