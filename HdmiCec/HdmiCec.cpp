...
//=========================================== HdmiCec =========================================
	...
	void HdmiCec::addDevice(const int logicalAddress) {
		...
		//If new cec device connected request for further dev details.
		//Even if device existing in current cache, Check device info is changed.
		//Since cec dev ping is initiated from App side and device reconnection can happen
		//independently and same logical address can be reassigned upon device reconnection.
		//A comcast setop boc can be reconnected with an apple tv in same port and both can
		//have same logical address.
